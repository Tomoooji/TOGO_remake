...
HCSR04::HCSR04(const int send_pulse_length, const int send_span_min, const int read_timeout):
      send_pulse_length(send_pulse_length), send_span_min(send_span_min), read_timeout(read_timeout), distance(0.0f){};
...
void HCSR04::begin(const uint8_t* trigPin, const uint8_t* echoPin){
  ...
  attachInterruptArg(digitalPinToInterrupt(*this->pin_echo),readPulseISR,this,CHANGE);
}

void HCSR04::begin(const uint8_t *pin){
  this->begin(&pin[0], &pin[1]);
}
...
void HCSR04::update(){
  if(this->status == HCSR04::WAITING && millis() - this->last_send_time > this->read_timeout){
    // timeout
    this->status = HCSR04::READY;
    this->last_send_time = 0;
    this->pulse = 0;
  ...
  if(this->status == HCSR04::READY){
    ...
    if(pulse>0)this->distance = this->calcDistance(this->pulse);
...
void IRAM_ATTR HCSR04::readPulseISR(void* arg){
  ...
  if(digitalRead(*self->pin_echo) == HIGH){
    ...
    if(self->status == HCSR04::READING){
      ...
      self->pulse = now>self->pulse? now-self->pulse:(2<<32)-1-self->pulse+now;
